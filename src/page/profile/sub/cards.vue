...
<section class="page cards">
	<div class="c-list">
		<div v-for="key in cardlist" @click="selectCard(key.serialno)" :class="[{ 'edit-card' : isEdit },{ 'on' : isSelected }]" class="card box bg-white">
			<div class="c-logo">
	...
	<div v-if="!isEdit" class="va">
		<div class="col">
			<router-link :to="{ name : 'add-card' }" class="va-body icon-add">添加银行卡</router-link>
		</div>
	</div>
	<div v-if="cardlist.length > 0" class="va box c-edit">		
		<a v-if="!isSelected && !isEdit" class="edit" @click="isEdit = true" href="javascript:;">编辑</a>
		<a v-if="isEdit" class="font-gray edit-off" @click="isSelected = false;isEdit = false" href="javascript:;">取消</a>
		<a v-if="isEdit" class="font-red" @click="delSedCard" href="javascript:;">删除</a>
	</div>
